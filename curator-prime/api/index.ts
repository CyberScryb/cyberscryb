...
app.post('/api/analyze-item', upload.array('images'), async (req, res) => {
  try {
    ...
    parsedResult.provenance = {
      ...parsedResult.provenance,
      digitalHash: '0x' + crypto.createHash('sha256').update(dataString).digest('hex'),
    };
  ...
  try {
    ...
    try {
      ...
      data = {
        ...
        keyInsight: 'Unable to find data.',
        demandLevel: 'Medium',
    ...
    const sources = chunks
      .flatMap((c: any) => (c.web?.uri ? [{ title: c.web.title || 'Source', url: c.web.uri }] : []))
      .slice(0, 5);
