...
function loadHighScore() {
    ...
    highScoreEl.textContent = highScores[gameMode] < 1000 ? `0${highScores[gameMode]}` : highScores[gameMode];
}
...
function draw() {
    ...
    snake.forEach((segment, idx) => {
        ...
        if (idx === 0) {
            ctx.fillStyle = '#1A1814'; // bright cyan
            ctx.shadowColor = '#1A1814';
            ctx.shadowBlur = 15;
        } else {
            // body segments fade slightly
            ctx.fillStyle = `rgba(26, 24, 20, ${Math.max(0.4, 1.0 - idx / (snake.length + 2))})`;
            ctx.shadowColor = '#1A1814';
            ctx.shadowBlur = 5;
        }
