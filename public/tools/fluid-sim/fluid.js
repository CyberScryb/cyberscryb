class FluidSolver {
  ...
  addSource(x, s) {
    for (let i = 0; i < this.size; i++) x[i] += this.dt * s[i];
  }
  setBnd(b, x) {
    const N = this.N;
    for (let i = 1; i <= N; i++) {
  ...
  velStep() {
    ...
    this.diffuse(1, this.u, this.u_prev, this.visc);
    [this.v_prev, this.v] = [this.v, this.v_prev];
    this.diffuse(2, this.v, this.v_prev, this.visc);
  ...
  densStep(d, d0) {
    this.addSource(d, d0);
    [d0, d] = [d, d0];
    this.diffuse(0, d, d0, this.diff);
    [d0, d] = [d, d0];
    this.advect(0, d, d0, this.u, this.v);
    return d;
  }
  step() {
    this.velStep();
    ...
    this.u_prev.fill(0);
    this.v_prev.fill(0);
    this.dens_r_prev.fill(0);
    this.dens_g_prev.fill(0);
    this.dens_b_prev.fill(0);
  }
  reset() {
    this.u.fill(0);
    this.v.fill(0);
