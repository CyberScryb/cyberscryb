...
**⛔ Pro activation BROKEN — diagnosed, NOT yet fixed (touches payments → awaiting Nathan's go).** `pro-success/` spins forever on "Activating Your Pro Access." Root cause in `functions/index.js` `validateStripeSession` (line ~1492): (1) **line 1510 reads the Stripe secret via `functions.config().stripe.secret` with NO `process.env` fallback** — unlike every other secret (GOOGLE_API_KEY line 366/605/1168, ANALYTICS_SECRET line 1333) which all have `|| process.env.X`. firebase-functions is **v6.3.2**, where `functions.config()` is dead → the Stripe read returns nothing while the AI tools keep working via their env fallback. (2) **No try/catch around the async handler** → any throw/rejection leaves the HTTP response unsent → infinite client spinner. (3) No timeout on the Stripe `https.request` or the client `fetch`. Proposed fix: add `|| process.env.STRIPE_SECRET` (mirror existing pattern), wrap handler in try/catch that always responds, add request + fetch timeouts. **Requires Nathan to set `STRIPE_SECRET` in the gitignored `functions/.env`** (same file holding GOOGLE_API_KEY) + a functions redeploy.
