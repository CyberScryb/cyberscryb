...
- **21 guide pages (json/csv/color/password/markdown/etc. utility guides) had a literal `data-ad-slot="auto"` placeholder** instead of a real numeric slot ID — confirmed via live console: hard `adsbygoogle.push() error: No slot size for availableWidth=0` `TagError`, meaning these ad requests never even fired. **The actual bug lived in `generate-pages.js`'s own page template (lines ~1281/1301)**, not just in the static files — these 21 pages are programmatically regenerated by `node generate-pages.js`, which now runs on every deploy (see next item). Fixed the template itself to use the real, proven slot `9198647442`.
- **New deploy-pipeline gotcha discovered this session**: `.github/workflows/deploy.yml` was changed (by an earlier, uncommitted-locally session — commit `2109f92`) to run `python3 sync_and_build.py` before `firebase deploy`. That means **`content-site/` is now the only real source of truth** — `public/` gets fully regenerated from it on every deploy, including guide pages re-templated from `generate-pages.js`. First deploy of this session's fix *looked* successful but got silently reverted by that exact build step, because the file-level fix hadn't touched the generator source yet. Second deploy (generator fixed) confirmed correct live.
- **Homepage, `/guides/`, and 21 flagship tool pages** (landlord-tenant-letter, insurance-denial-appeal, payment-demand-letter, utility-shutoff-letter, both support calculators, hoa-dispute-letter, security-deposit-demand, etc.) loaded the AdSense library script but had **zero manual ad units** — relying entirely on Google Auto Ads, which live-testing showed returns `unfilled` on pages with no manual unit backing it up. Added the site's standard below-content/after-footer manual unit (slot `9198647442`) to all of them, plus 3 fresh units Nathan pulled from AdSense (homepage responsive `1250796004`, homepage autorelaxed `6553621393`, in-article fluid `2899271242` used on 6 short guide articles).
- **Verified live** (fresh no-cache loads, not just deploy success): correct slot IDs served, zero JS errors, real non-zero ad container widths, `push()` completing cleanly. Note: "unfilled" ad status on some impressions is normal auction variance, not a bug — a real filled ad (712×427px) was directly observed rendering on a blog post mid-session.
- Also discovered mid-session: local `main` was 2 commits behind `origin/main` (a homepage redesign to an intent-based finder UI, `1ccac29`, plus the deploy-pipeline change above) that predated this session but hadn't been fetched yet — caused a merge conflict on `content-site/index.html`, resolved by keeping the redesign fully intact and re-adding the ad units into its new structure.

**Not done / flagged for Nathan:** separate Dependabot alert on push — 10 vulnerabilities (4 high, 6 moderate) on `main`, unreviewed this session. `about.html`/`contact.html`/`disclosure.html`/`privacy.html`/`terms.html` deliberately left with no ad units (low-value/legal pages, not a bug).

**Rule learned:** `content-site/` is the sole real source of truth as of the `2109f92` pipeline change — `public/` is a build artifact regenerated by `sync_and_build.py`/`generate-pages.js` on every deploy. Don't trust a fix that only touched `public/`, and don't trust deploy success alone — verify live with a cache-busted fetch, since both browser cache (`max-age=3600`) and this repo's own generator can silently revert a file-level fix.
...
- `privacy-generator`'s `h2 { color: #111111 }` — left alone on purpose, it's inside the JS template that generates the _downloadable_ privacy-policy document (white background there already), not the live dark-theme bug.
...
**⚠️ DEPLOY BLOCKER (Nathan action):** GitHub Actions on the CyberScryb org has been failing since Aug 3 with "The job was not started because your account is locked due to a billing issue" — every deploy job dies in ~2s with no steps (last good deploy Jul 23 = why live was stale; GitHub _Pages_ deploys still green, but the site ships via the **Firebase** workflow). This is **github.com billing** (repo-owner account), NOT Google Cloud (GCP verified healthy, $0 due, $10 credit). Check github.com → CyberScryb org → Settings → Billing and plans. GCP side note: "My Billing Account 3" shows −$7.18 balance w/ Aug 1 invoice — covered by credit, but keep an eye on it.
