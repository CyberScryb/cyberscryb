...
dependency_graph:
  ...
  provides:
    [
      'jest coverage for AI_PROMPTS dispatch table',
      'regression guard against res.send() errors',
      'test export hook in functions/index.js',
    ]
...
tech_stack:
  ...
  patterns:
    [
      'jest.mock for firebase-admin/firebase-functions',
      'describe.each for parametric tool tests',
      'CommonJS __testing export guarded by NODE_ENV',
    ]
key_files:
  created:
