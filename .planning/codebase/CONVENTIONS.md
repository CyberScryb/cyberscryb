...
document.addEventListener('DOMContentLoaded', () => {
    window.CSAITool.init({
        ...
        collectInput: () => {          // Returns the raw input string
            return document.getElementById('tool-input').value.trim();
        },
        collectParams: () => {         // Returns extra params object (optional)
            return { key: value };
        },
        onStats: (text) => {           // Called after output renders (optional)
...
--text-faint: #555555
--accent: #34F5C5
--accent-glow: rgba(52, 245, 197, 0.5)
