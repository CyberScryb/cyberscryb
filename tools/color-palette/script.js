...
function renderPalette() {
  ...
  container.innerHTML = currentPalette
    .map(
      (c, i) => `
...
function switchExport(mode) {
  ...
  document
    ...
    .forEach(t => t.classList.toggle('active', t.textContent.toLowerCase().includes(mode)));
  renderExport();
}
...
function renderExport() {
  ...
  switch (currentExportMode) {
    case 'css':
      code =
        ':root {\n' +
        currentPalette.map((c, i) => `  --color-${i + 1}: ${c.hex};`).join('\n') +
        '\n}';
      break;
    ...
    case 'tailwind':
      code =
        'module.exports = {\n  theme: {\n    extend: {\n      colors: {\n        palette: {\n' +
        currentPalette.map((c, i) => `          ${i + 1}: '${c.hex}',`).join('\n') +
        '\n        }\n      }\n    }\n  }\n}';
      break;

    case 'scss':
      code = currentPalette.map((c, i) => `$color-${i + 1}: ${c.hex};`).join('\n');
      break;

    case 'array':
      code = JSON.stringify(
        currentPalette.map(c => c.hex),
        null,
        2
      );
      break;
