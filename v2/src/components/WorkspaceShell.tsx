...
export const ToolShell = ({
  ...
  return (
    <div className="flex flex-col md:flex-row h-full bg-base animate-in fade-in duration-300">
      <Helmet>
        <title>
          {config.name} — {config.aiFeatures?.length > 0 ? 'Local + AI' : '100% Local, No Server'} |
          CyberScryb
        </title>
        <meta
          name="description"
          content={`Free client-side ${config.name.toLowerCase()}. ${config.description} ${config.aiFeatures?.length > 0 ? 'Includes optional AI.' : 'No tracking, no uploads.'}`}
        />
        <meta
          property="og:title"
          content={`${config.name} — ${config.aiFeatures?.length > 0 ? 'Local + AI' : '100% Local'} | CyberScryb`}
        />
        <meta
          property="og:description"
          content={`Free client-side ${config.name.toLowerCase()}. ${config.description}`}
        />
        <meta
          property="og:image"
          content={`data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><rect width="1200" height="630" fill="%2309090b"/><rect x="50" y="50" width="1100" height="530" rx="20" fill="%23121214" stroke="%2327272a" stroke-width="2"/><text x="100" y="200" font-family="monospace" font-size="60" font-weight="bold" fill="%23f4f4f5">${encodeURIComponent(config.name)}</text><text x="100" y="280" font-family="sans-serif" font-size="30" fill="%23a1a1aa">${encodeURIComponent(config.description)}</text><text x="100" y="500" font-family="monospace" font-size="40" fill="%2334f5c5" font-weight="bold">CYBERSCRYB ${encodeURIComponent(config.category.toUpperCase())}</text></svg>`}
        />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex flex-col px-6 py-4 border-b border-subtle bg-elevated/50 backdrop-blur-xl shrink-0 z-10 relative">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-[10px] font-mono text-muted uppercase tracking-widest flex items-center gap-2 px-2 py-0.5 bg-surface rounded border border-subtle">
              ...
              {config.aiFeatures?.length > 0 ? 'Local + Optional Cloud AI' : '100% Local Execution'}
            </div>
          </div>
          ...
          <AnimatePresence>
            {sampleBanner && (
              ...
              >
                Demo loaded.{' '}
                <button className="underline hover:text-white" onClick={onDismissSample}>
                  Clear
                </button>
              </motion.div>
            )}
        ...
        <div className="flex-1 overflow-hidden flex flex-col xl:flex-row relative z-0">
          ...
          <div className="flex-1 relative z-10 overflow-hidden flex">{children}</div>
            ...
            <div className="w-full xl:w-72 border-t xl:border-t-0 xl:border-l border-subtle bg-surface flex flex-col shrink-0 z-20 overflow-y-auto">
              {rightSidebar.whyItMatters && (
                <div className="p-4 border-b border-strong">
                  <h3 className="text-[10px] font-bold text-primary font-mono uppercase tracking-widest mb-2">
                    Why this matters
                  </h3>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {rightSidebar.presets.map((p: any, i: number) => (
                      <button
                        key={i}
                        onClick={p.onClick}
                        className="text-left py-1.5 px-2 rounded hover:bg-elevated text-xs text-muted hover:text-primary transition-colors border border-transparent hover:border-subtle truncate"
                      >
                        {p.label}
                      </button>
                    ))}
                ...
                <div className="p-4 border-b border-strong">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {toolExamples.map((ex, i) => (
                      <Link
                        key={i}
                        to={`/tools/${config.slug}/examples/${ex.exampleSlug}`}
                        className="text-left py-1.5 px-2 rounded hover:bg-elevated text-xs text-muted hover:text-primary transition-colors border border-transparent hover:border-subtle truncate block"
                      >
                        {ex.h1
                  ...
                  <div className="flex flex-col gap-2">
                    {rightSidebar.shortcuts.map((s: any, i: number) => (
                      <div key={i} className="flex items-center justify-between text-xs text-muted">
                        <span>{s.label}</span>
                        <kbd className="font-mono bg-elevated px-1 border border-subtle rounded">
                          {s.key}
                        </kbd>
                      </div>
                    ))}
    ...
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 flex flex-col border-b lg:border-b-0 lg:border-r border-subtle bg-base/50 min-h-[50vh] lg:min-h-0 relative">
        <div className="flex justify-between items-center px-4 py-3 bg-surface border-b border-subtle z-10">
          <span className="text-xs font-bold font-mono text-[#34F5C5] uppercase tracking-widest">
            {leftConfig.title || 'INPUT'}
          </span>
          <div className="flex gap-2">{leftConfig.actions}</div>
        </div>
        <div className="flex-1 relative overflow-auto hide-scrollbar p-0">{left}</div>
      </div>
      <div className="flex-1 flex flex-col bg-base/50 min-h-[50vh] lg:min-h-0 relative">
        <div className="flex justify-between items-center px-4 py-3 bg-surface border-b border-subtle z-10">
          <span className="text-xs font-bold font-mono text-indigo-400 uppercase tracking-widest">
            {rightConfig.title || 'OUTPUT'}
          </span>
          <div className="flex gap-2">{rightConfig.actions}</div>
        </div>
