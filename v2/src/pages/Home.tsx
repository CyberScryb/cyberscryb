  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="A powerful suite of client-side developer tools. No tracking, minimal server dependency, optional AI features. Privacy first."
        />
      </Helmet>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-base/80 backdrop-blur-sm p-4">
            ...
            >
              ...
              <p className="text-muted leading-relaxed mb-8">
                You are about to use a suite of tools that do not send your data to a server. We
                don't want your telemetry, and we don't want to store your secrets. Before you
                begin, you have a choice.
              </p>
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => handleDismissManifesto('read')}
                  className="w-full py-4 bg-accent text-black font-bold rounded-lg hover:bg-white transition-colors flex items-center justify-center gap-2"
                >
                  <BookOpen size={18} /> Read the Manifesto
                </button>
                <button
                  onClick={() => handleDismissManifesto('skip')}
                  className="w-full py-4 bg-transparent border border-subtle text-muted hover:text-primary hover:border-strong font-bold rounded-lg transition-colors"
                >
                  I just want tools
                </button>
              </div>
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center text-center gap-6">
          ...
          <p className="text-lg md:text-xl text-muted max-w-2xl mx-auto mb-10 leading-relaxed">
            Stop pasting your production secrets into random ad-filled websites. Run everything
            offline, securely, directly in your browser.
          </p>
          ...
          <div className="w-full max-w-xl mx-auto bg-elevated border border-strong rounded-xl p-4 shadow-2xl flex items-center gap-3 relative group overflow-hidden">
            ...
            <div className="flex-1 flex items-center gap-1 text-left">
              ...
              <kbd className="px-1.5 py-0.5 bg-surface border border-subtle rounded text-xs fold-bold font-mono text-primary">
                ⌘K
              </kbd>
      ...
      <div className="bg-surface border-b border-subtle py-8">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
              <h4 className="font-bold text-primary font-mono uppercase tracking-widest text-xs mb-1">
                Zero Telemetry
              </h4>
              <p className="text-muted text-sm leading-relaxed">
                Nothing leaves your machine. Your payloads stay in your RAM.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
              ...
              <p className="text-muted text-sm leading-relaxed">
                No network round-trips. Powered by local Web Workers and WASM.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
              ...
              <p className="text-muted text-sm leading-relaxed">
                Built on standardized browser APIs for long-term stability.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="sticky top-[4.5rem] z-30 bg-base/80 backdrop-blur-xl border border-subtle rounded-xl p-4 shadow-xl flex flex-col lg:flex-row gap-4 justify-between items-center transition-all">
            <div className="relative w-full lg:max-w-md">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1">
                <kbd className="w-6 h-6 flex items-center justify-center bg-surface border border-subtle rounded text-xs text-muted font-sans font-medium">
                  /
                </kbd>
              </div>
            </div>
            ...
            <AnimatePresence>
              {filteredTools.map(tool => (
                <Link
                  to={'/tools/' + tool.slug}
                  key={tool.id}
                  className="block outline-none rounded-xl focus-ring"
                >
                  <motion.div
                    layout
                  ...
                  >
                    ...
                    <p className="text-sm text-muted leading-relaxed font-normal mb-8 relative z-10 line-clamp-2">
                      {tool.description}
                    </p>
                    ...
                    <div className="relative z-10 mt-auto flex items-center justify-between w-full">
                      <div className="flex items-center gap-1.5 text-xs font-mono text-muted/70 group-hover:text-accent transition-colors">
                        ...
                        {tool.aiFeatures?.length > 0 ? 'local + optional AI' : 'process locally'}
                      </div>
                    </div>
                  </motion.div>
                </Link>
