  ...
  if (!post) {
    return (
      <div className="py-24 px-6 max-w-3xl mx-auto text-center">
        ...
        <Link to="/blog" className="text-[#34F5C5] hover:underline font-bold">
          Return to Blog
        </Link>
      </div>
    );
  ...
  return (
    <article className="py-12 md:py-24 px-6 max-w-4xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <p className="text-xl text-muted leading-relaxed max-w-3xl">{post.snippet}</p>
      </div>

      <div
        className="w-full aspect-video rounded-3xl overflow-hidden shadow-xl border border-subtle mb-16 cursor-pointer group"
        onClick={e => {
      ...
      >
        <img
          src={post.image}
          alt={post.title}
          className="w-full h-full object-cover mix-blend-luminosity group-hover:mix-blend-normal hover:scale-105 transition-all duration-700 group-hover:drop-shadow-[0_0_15px_rgba(52,245,197,0.3)]"
        />
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 bg-surface/50 p-8 rounded-3xl border border-white/5">
          <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-[#34F5C5] to-indigo-500 shrink-0 p-1">
            <div className="w-full h-full rounded-full bg-base overflow-hidden">
              <img
                src="https://images.unsplash.com/photo-1544725176-7c40e5a71c5e?auto=format&fit=crop&q=80&w=200"
                alt="Author"
          ...
          <div>
            ...
            <p className="text-muted leading-relaxed">
              We build privacy-first, purely client-side developer tools. No servers, no tracking,
              just fast software.
            </p>
          </div>
        </div>
