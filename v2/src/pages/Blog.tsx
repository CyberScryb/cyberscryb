  ...
  return (
    <div className="py-24 px-6 max-w-3xl mx-auto">
      <Helmet>
        ...
        <meta
          name="description"
          content="Technical deep dives on building local-first, privacy-respecting client-side tools."
        />
      ...
      <p className="text-muted text-lg mb-12">
        Building high-performance, privacy-first web tools.
      </p>
      ...
      <div className="space-y-12">
        {POSTS.map((p, i) => (
          <Link
            to={`/blog/${p.slug}`}
            key={i}
            className="block group border border-subtle hover:border-[#34F5C5]/30 bg-elevated hover:bg-surface p-6 rounded-2xl transition-all duration-300 cursor-pointer overflow-hidden relative shadow-sm hover:shadow-md"
          >
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="w-full md:w-1/3 shrink-0 rounded-xl overflow-hidden shadow border border-white/5 relative aspect-video md:aspect-square">
                <img
                  src={p.image}
                  alt={p.title}
                  className="w-full h-full object-cover mix-blend-luminosity hover:mix-blend-normal transition-all duration-500 group-hover:scale-105"
                />
              </div>
              <div className="flex flex-col h-full justify-between w-full">
                <div>
                  ...
                  <h2 className="text-2xl font-bold mb-3 group-hover:text-[#34F5C5] transition-colors">
                    {p.title}
                  </h2>
