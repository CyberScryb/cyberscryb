...
export const MediaTools = [
  {
    ...
    desc: 'Client-side EXIF viewer & stripper. Preserve privacy.',
    ...
    component: function ImgTool({ onClose, config }: any) {
      ...
      const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = e => {
          ...
          try {
            ...
            const hasExif = Object.values(exifObj).some(
              (val: any) => typeof val === 'object' && Object.keys(val).length > 0
            );
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "Your iPhone embeds GPS coordinates in every photo by default. EXIF analysis and stripping should happen locally—uploading an image to 'clean' it defeats the purpose of privacy.",
          }}
        >
          <div className="flex flex-col md:flex-row h-full">
            <div className="w-full md:w-1/3 border-r border-subtle bg-surface p-6 flex flex-col gap-6 overflow-auto">
              <div>
                ...
                <Button
                  ...
                  className="w-full h-12 shadow-sm text-base"
                >
                  Upload JPEG Image
                </Button>
              </div>
                  ...
                  {exif ? (
                    <div className="bg-elevated border border-warning/20 p-4 rounded-xl">
                      <h3 className="text-warning font-bold flex items-center gap-2 mb-2">
                        <ShieldCheck size={16} /> Metadata Detected
                      </h3>
                      <p className="text-xs text-muted mb-4">
                        This image contains EXIF metadata, which may include location, timestamps,
                        and camera details.
                      </p>
                      <div className="mb-4">
                        <AIInlineExplanation
                          prompt="Explain what EXIF data is and why someone would want to strip it for privacy reasons."
                          context="-"
                    ...
                    <div className="bg-elevated border border-accent/20 p-4 rounded-xl">
                      <h3 className="text-accent font-bold flex items-center gap-2 mb-2">
                        <ShieldCheck size={16} /> Clean Image
                      </h3>
                      <p className="text-xs text-muted">
                        No EXIF metadata was found. This image is safe to share.
                      </p>
                    </div>
                  )}
                </>
              ...
              {strippedData && (
                <div className="bg-accent/10 border border-accent/20 p-4 rounded-xl">
                  ...
                  <a
                    href={strippedData}
                    download="stripped_image.jpg"
                    className="w-full flex items-center justify-center p-2 bg-accent text-base rounded font-bold transition-all hover:bg-white text-black"
                  >
                    Download Safe Image
                  </a>
                </div>
            ...
            <div className="flex-1 flex items-center justify-center p-6 bg-base bg-[radial-gradient(rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:20px_20px] overflow-hidden relative">
              {imgData ? (
                <img
                  src={strippedData || imgData}
                  alt="Preview"
                  className="max-w-full max-h-full object-contain drop-shadow-2xl rounded"
                />
              ) : (
                <div
                  className="text-muted flex flex-col items-center opacity-50 cursor-pointer hover:opacity-100 transition-opacity"
                  onClick={() => fileInput.current?.click()}
                >
  ...
  },
  {
    id: 'svg',
    title: 'SVG Optimizer',
    desc: 'Minify and optimize SVGs using SVGO patterns.',
    icon: <ImgIcon size={18} />,
    tag: 'Media',
    component: function SvgTool({ onClose, config }: any) {
      const [{ input }, setState, shareUrl, copyShareLink] = useToolState(config.id, {
        input:
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M12 2L2 22h20L12 2z"/></svg>',
      });
      const ai = useAI();
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              'Inline SVGs prevent extra network requests and allow CSS manipulation. But raw SVGs from Figma are bloated with metadata and useless transforms.',
            presets: [
              {
                label: 'Sample Icon',
                onClick: () => {
          ...
          <SplitPane
              ...
              <div className="flex flex-col h-full bg-transparent p-0 relative">
                ...
                {(ai.isLoading || aiResult) && (
                  <div className="absolute bottom-4 left-4 right-4 z-10 bg-indigo-500/10 border border-indigo-500/20 p-4 rounded-lg backdrop-blur-md">
                    {ai.isLoading ? (
                      <span className="animate-pulse text-indigo-400 font-bold text-xs uppercase tracking-widest">
                        Analyzing SVG...
                      </span>
                    ) : (
                      <>
                        <div className="font-bold text-indigo-300 text-sm mb-1">
                          {aiResult.componentName}
                        </div>
                        <div className="text-xs text-indigo-200/70 mb-2">
                          aria-label: "{aiResult.ariaLabel}"
                        </div>
            ...
            right={
              <div className="flex items-center justify-center h-full bg-surface">
                <img
                  src={`data:image/svg+xml;utf8,${encodeURIComponent(input)}`}
                  className="w-32 h-32"
                  alt="SVG Preview"
                />
              </div>
    ...
    component: function QrTool({ onClose, config }: any) {
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "Most 'free' free QR generators inject tracking redirects into your payload. Generate them client-side in Canvas. It's safe, instantly rendered, and permanent.",
          }}
        >
          <div className="flex flex-col md:flex-row h-full">
            <div className="w-full md:w-1/3 border-r border-subtle bg-surface p-6 space-y-6">
              <div>
                <label className="text-xs uppercase text-muted font-bold block mb-2">
                  Payload URL/Text
                </label>
                <textarea
                  className="w-full bg-elevated border border-strong rounded p-3 text-primary focus:outline-none focus:border-[#34F5C5]/50 h-32"
                  value={input}
                  onChange={e => {
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 rounded border border-subtle cursor-pointer"
                />
              </div>
            </div>
  ...
  },
  {
    ...
    desc: 'Harmonies, WCAG Contrast Grid, exports.',
    icon: <Palette size={18} />,
    tag: 'Design',
    component: function ColorTool({ onClose, config }: any) {
      const [{ hex }, setState, shareUrl, copyShareLink] = useToolState(config.id, {
        hex: '#34f5c5',
      });
      const ai = useAI();
      ...
      const handleGenerateMood = () => {
        const mood = prompt(
          "Enter a mood or scene (e.g., 'forest twilight calm', 'cyberpunk neon')"
        );
        if (!mood) return;
        ai.run({
              ...
              colors: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    role: {
                      type: 'string',
                      description: 'e.g. primary, secondary, accent, background',
                    },
                  },
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "A palette isn't just colors, it's a hierarchy. You need a primary brand, a dark base, a light contrast, and a punchy accent. AI gets this right better than color wheel math.",
          }}
        >
                ...
                {palettes.map((p, idx) => (
                  <div key={idx} className="bg-surface border border-subtle rounded-xl p-6">
                    <h3 className="text-xl font-bold tracking-tight mb-4 flex items-center justify-between">
                      ...
                      <span className="text-xs font-mono text-muted uppercase bg-base px-2 py-1 rounded">
                        AI GENERATED
                      </span>
                    </h3>
                      ...
                      {p.colors.map((c: any) => (
                        ...
                        >
                          <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="text-white font-mono text-xs uppercase font-bold">
                              {c.hex}
                            </div>
                            <div className="text-white/80 text-[10px] break-words leading-tight">
                              {c.name}
                            </div>
                          </div>
                        </div>
            ...
            {palettes.length === 0 && (
              <>
                <input
                  ...
                  className="w-32 h-32 rounded-xl border-4 border-subtle shadow-xl cursor-pointer"
                />
                <div className="mt-8 text-3xl font-mono uppercase text-primary tracking-widest">
                  {hex}
                </div>
                <div className="mt-12 flex gap-4">
                  {[1, 2, 3, 4, 5].map(i => (
                    <div
                      ...
                      style={{ backgroundColor: hex, opacity: 1 - i * 0.15 }}
                    />
                  ))}
                </div>
              </>
