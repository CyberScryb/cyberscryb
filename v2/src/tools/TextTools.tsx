...
export const TextTools = [
  {
    ...
    desc: 'Evade detectors by normalizing phrasing and entropy. Compares diff.',
    icon: <Type size={18} />,
    tag: 'Text',
    component: function AntiAiTool({ onClose, config }: any) {
      const [{ input }, setState, shareUrl, copyShareLink] = useToolState(config.id, {
        input:
          'Furthermore, it is pivotal to delve into the multifaceted paradigm of synergistic leverage in a ubiquitous ecosystem.',
      });
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "LLMs have distinct lexical fingerprints. Words like 'delve' and 'robust' are dead giveaways. Normalizing entropy makes text un-detectable but actually legible for humans.",
          }}
        >
          <SplitPane
            left={
              <CodeEditor
                ...
                placeholder="Paste overly-robotic text here..."
              />
            }
            ...
            right={
              <div className="h-full overflow-auto p-6 bg-surface/50 text-primary markdown-body">
                {ai.isLoading && !ai.streamData && (
                  <div className="text-muted tracking-widest uppercase text-xs animate-pulse font-bold">
                    Humanizing...
                  </div>
                )}
                {ai.streamData && (
                ...
                {output && !ai.streamData && !ai.isLoading && (
                  <MarkdownPreview
                    source={output}
                    style={{ backgroundColor: 'transparent', color: 'inherit' }}
                  />
                ...
                {!output && !ai.streamData && !ai.isLoading && (
                  <div className="text-muted h-full flex items-center justify-center">
                    Click Humanize to rewrite text.
                  </div>
                )}
              </div>
            }
            rightConfig={{
              ...
              actions: (
                <ActionButton
                  onClick={process}
                  label={ai.isLoading ? 'Humanizing...' : 'Humanize (AI)'}
                  primary
  ...
  },
  {
    id: 'lorem',
    title: 'Lorem Ipsum++',
    desc: 'Standard, hipster, corporate, hacker jargon profiles.',
    icon: <FileText size={18} />,
    tag: 'Text',
    component: function LoremTool({ onClose, config }: any) {
      const [{ type, context }, setState, shareUrl, copyShareLink] = useToolState(config.id, {
        type: 'standard',
        context: 'A luxury watch brand',
      });
      const [output, setOutput] = useState(
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              'Client presentations fail when the latin filler is too distracting. Context-aware placeholder text allows stakeholders to evaluate the layout hierarchy without fixating on the copy.',
          }}
        >
          <div className="flex flex-col h-full bg-base p-8 gap-6 max-w-5xl mx-auto w-full">
            <div className="flex gap-4 p-6 bg-surface border border-subtle rounded-xl shadow-sm items-end flex-wrap">
              <div className="space-y-2">
                ...
                <select
                  className="bg-elevated border border-strong rounded px-4 py-2.5 text-sm text-primary focus:outline-none focus:border-[#34F5C5] transition-colors"
              ...
              {type === 'contextual' && (
                <div className="space-y-2 flex-1 min-w-[300px]">
                  ...
                  <input
                    className="w-full bg-elevated border border-strong rounded px-4 py-2.5 text-sm text-primary focus:outline-none focus:border-[#34F5C5] transition-colors"
                    placeholder="e.g. A high-end luxury watch brand"
                    value={context}
            ...
            <div className="flex-1 bg-elevated border border-strong rounded-xl p-8 font-serif text-lg leading-relaxed text-primary/80 overflow-auto shadow-inner">
              {ai.isLoading && !ai.streamData && (
                <span className="opacity-50 animate-pulse text-sans text-sm font-bold uppercase tracking-widest">
  ...
  },
  {
    id: 'text-toolkit',
    title: 'Text Toolkit',
    desc: 'Case converter, slugify, sort, dedupe, count.',
    icon: <Type size={18} />,
    tag: 'Text',
            ...
            rightConfig={{
              actions: (
                <div className="flex gap-2 flex-wrap">
                  <button
                    className="text-[10px] font-bold uppercase tracking-widest bg-surface border border-subtle px-3 py-1.5 rounded hover:bg-[#34F5C5]/10 hover:border-[#34F5C5]/30 hover:text-[#34F5C5] text-primary transition-all shadow-sm"
                  ...
                  <button
                    className="text-[10px] font-bold uppercase tracking-widest bg-surface border border-subtle px-3 py-1.5 rounded hover:bg-[#34F5C5]/10 hover:border-[#34F5C5]/30 hover:text-[#34F5C5] text-primary transition-all shadow-sm"
                    onClick={() => setOutput(input.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}
                  >
                    Slugify
                  </button>
                  <button
                  ...
                  <button
                    className="text-[10px] font-bold uppercase tracking-widest bg-surface border border-subtle px-3 py-1.5 rounded hover:bg-[#34F5C5]/10 hover:border-[#34F5C5]/30 hover:text-[#34F5C5] text-primary transition-all shadow-sm"
                    onClick={() => setOutput(Array.from(new Set(input.split('\n'))).join('\n'))}
                  >
                    Dedupe
                  </button>
                </div>
