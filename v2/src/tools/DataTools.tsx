...
export const DataTools = [
  {
    ...
    desc: 'Interactive graph visualization of nested JSON structures.',
    ...
    component: function JsonVisTool({ onClose, config }: any) {
      ...
      const generateSample = () => {
        const desc = prompt(
          "What kind of sample JSON do you want? (e.g., 'User profile with 3 latest orders')"
        );
        if (!desc) return;
        setAiMode('generate');
        setShowSample(false);
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "Most JSON parsers just fail on commas. Real tooling lets you inspect the AST and reshape it instantly. Your data layer isn't clean until you can read it without squinting.",
            presets: [
              {
              ...
              <div className="flex flex-col h-full bg-transparent p-0 relative">
                ...
                {!ai.isLoading && !ai.streamData && aiMode !== 'generate' ? (
                  ...
                  <div className="flex flex-col h-full">
                    {ai.isLoading && (
                      <div className="p-6 text-xs uppercase tracking-widest text-indigo-400 font-bold mb-4 animate-pulse">
                        Generating sample...
                      </div>
                    )}
                    <CodeEditor value={out} readOnly />
  ...
  {
    id: 'timestamp',
    title: 'Timestamp Studio',
    desc: 'Epoch conversions, ISO8601 formatting, math.',
    icon: <Clock size={18} />,
    tag: 'Data',
    component: function TimeTool({ onClose, config }: any) {
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              'Timezones are a myth invented by bugs. UTC epoch strings are the only source of truth. Default everything globally to ms since 1970.',
          }}
        >
          <div className="p-8 space-y-4">
            <input
              className="w-full bg-surface p-4 text-xl font-mono text-center rounded border-subtle border"
              value={input}
              onChange={e => {
            ...
            />
            <div className="text-center font-mono text-lg text-[#34F5C5]">
              {isNaN(d.getTime()) ? 'Invalid Date' : d.toISOString()}
            </div>
            {!isNaN(d.getTime()) && (
              <AIInlineExplanation
                prompt={'Explain the components of this ISO-8601 string.'}
                context={d.toISOString()}
                label="Explain this ISO Format (AI)"
              />
            )}
          </div>
        </ToolShell>
    ...
    component: function JsonCsvTool({ onClose, config }: any) {
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "Data moves when it's flat. Excel rules the corporate world, while JSON dominates the API space. Being able to shift flawlessly without sending data to a remote server saves hours of PII anxiety.",
            presets: [
              {
                label: 'Sample Array',
                onClick: () => {
            ...
            leftConfig={{
              ...
              actions: (
                <ActionButton
                  ...
                  onClick={() => {
                    try {
                      setState(s => ({ ...s, input: JSON.stringify(JSON.parse(input), null, 2) }));
                      setShowSample(false);
                    } catch (e) {}
                  }}
                />
              ...
              actions: (
                <>
                  <select
                    ...
                    onChange={e => {
                      setState(s => ({ ...s, mode: e.target.value as any }));
                      setShowSample(false);
                    }}
                    value={mode}
                  >
    ...
    component: function DiffTool({ onClose, config }: any) {
      ...
      const summarizeDiff = () => {
        ...
        const diffStr = differences
          .map((p, i) => `${i + 1} | ` + (p.added ? '+ ' : p.removed ? '- ' : '  ') + p.value)
          .join('');
        ai.run({
          model: 'gemini-3.1-pro',
...
\`\`\``,
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              "Code review isn't just looking at red and green lines, it's about semantic intent. Local diff viewers ensure your uncommitted IP never hits an external server.",
            presets: [
              {
                label: 'Sample Function Fix',
                onClick: () => {
                  setState({
                    oldText: 'function sum(a,b) { return a+b; }',
                    newText: 'const sum = (a, b) => a + b;',
                  });
                  setShowSample(false);
                },
              },
          ...
          <SplitPane
            left={
              <div className="flex flex-col h-full gap-2 p-2 relative w-full">
                <textarea
                  className="flex-1 w-full bg-surface border border-subtle p-3 rounded font-mono text-sm text-primary resize-none focus:outline-none"
                  value={oldText}
                  onChange={e => {
                    setState(s => ({ ...s, oldText: e.target.value }));
                    setShowSample(false);
                  }}
                ...
                <textarea
                  ...
                  value={newText}
                  onChange={e => {
                    setState(s => ({ ...s, newText: e.target.value }));
                    setShowSample(false);
                  }}
            ...
            right={
              <div className="flex flex-col h-full relative">
                ...
                {showAi ? (
                  <div
                    className="p-6 h-full overflow-auto bg-indigo-500/5 text-indigo-100 markdown-body"
                    style={{ backgroundColor: 'transparent' }}
                  >
                  ...
                  <div className="p-6 font-mono text-sm leading-relaxed overflow-auto overflow-x-auto whitespace-pre">
                    {differences.map((part, index) => (
                      <span
                        ...
                        className={
                          part.added
                            ? 'bg-[#34F5C5]/20 text-[#34F5C5] px-1 rounded'
                            : part.removed
                              ? 'bg-red-500/20 text-red-400 px-1 rounded line-through'
                              : 'text-muted'
                        }
                      >
                        {part.value}
