...
export const WebTools = [
  {
    id: 'curl-code',
    title: 'cURL ↔ Code',
    desc: 'Convert cURL commands to fetch, axios, Python, Go, Rust.',
    icon: <Code2 size={18} />,
    tag: 'Dev',
    component: function CurlTool({ onClose, config }: any) {
      const [{ input, lang }, setState, shareUrl, copyShareLink] = useToolState(config.id, {
        input:
          'curl https://api.example.com -H "Authorization: Bearer my-token" -d \'{"key":"value"}\'',
        lang: 'Fetch',
      });
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              'Most API docs only provide cURL examples. AI makes syntax translation flawless without needing specific parsing libraries for every target language.',
          }}
        >
          <SplitPane
            ...
            left={
              <div className="h-full relative">
                ...
                />
                <div className="absolute bottom-4 left-4 right-4 z-10 p-2 bg-base rounded-md border border-subtle shadow overflow-y-auto max-h-[150px]">
                  <AIInlineExplanation
                    prompt="Explain what this curl command does, the method, headers, and the body in a human readable way."
                    context={input}
                    label="Explain this cURL (AI)"
                  />
                </div>
              </div>
            }
            rightConfig={{
              ...
              actions: (
                <>
                  <select
                    className="bg-elevated border border-strong rounded px-2 py-1 text-xs text-primary focus:outline-none font-sans font-medium"
                    onChange={e => {
                      setState(s => ({ ...s, lang: e.target.value }));
                      setShowSample(false);
                    }}
                    value={lang}
                  >
                  ...
                  <ActionButton
                    onClick={convert}
                    primary
                    label={ai.isLoading ? 'Converting...' : 'Convert'}
                  />
                </>
              ),
            }}
  ...
  {
    id: 'gig',
    title: 'Gig Auto-Pilot',
    desc: 'Automate freelance tasks & client proposals.',
    icon: <Globe size={18} />,
    tag: 'Productivity',
    component: function GigTool({ onClose, config }: any) {
      ...
      return (
        <ToolShell config={config} onClose={onClose}>
          <SplitPane
            ...
            rightConfig={{
              ...
              actions: (
                <ActionButton
                  onClick={generate}
                  primary
                  label={ai.isLoading ? 'Generating...' : 'Generate Proposal'}
                />
              ),
            }}
            right={
              <div className="h-full overflow-auto p-4 bg-surface/50 markdown-body">
                {output && !ai.isLoading && !ai.streamData && (
                  <MarkdownPreview
                    source={output}
                ...
                {ai.isLoading && !ai.streamData && (
                  <span className="opacity-50">Drafting proposal...</span>
                )}
                {ai.streamData && (
                  <MarkdownPreview
  ...
  {
    id: 'privacy',
    title: 'Legal Boilerplate Studio',
    desc: 'Generate Privacy Policies & ToS locally.',
    icon: <Globe size={18} />,
    tag: 'Legal',
    component: function PrivacyTool({ onClose, config }: any) {
      const [{ company, website, email, type }, setState, shareUrl, copyShareLink] = useToolState(
        config.id,
        {
          company: 'Acme Corp',
      ...
      return (
        <ToolShell
          ...
          rightSidebar={{
            whyItMatters:
              'Most online generators harvest your company intel to sell to marketing lists. LLMs know standard web legalese natively.',
          }}
        >
          <SplitPane
            ...
            left={
              <div className="p-6 space-y-4 text-sm bg-surface/30 h-full overflow-auto flex flex-col">
                <div>
                  <label className="block text-muted mb-1 text-xs uppercase tracking-widest font-bold">
                    Document Type
                  </label>
                  <select
                    className="w-full bg-elevated border-strong border rounded-lg p-3 text-primary focus:border-accent focus:ring-1 focus:ring-accent focus:outline-none transition-all"
                    value={type}
                ...
                {!output && !ai.isLoading && (
                  <div className="text-muted flex flex-col gap-2 items-center justify-center h-full text-center">
                    ...
                    <p className="text-xs max-w-xs">
                      Disclaimer: This tool generates templates and does not constitute formal legal
                      counsel.
                    </p>
                  </div>
                )}
                {ai.isLoading && (
                  <div className="opacity-50 animate-pulse mt-4">Drafting legal clauses...</div>
                )}
                {output && !ai.isLoading && (
                  <MarkdownPreview
                    source={output}
  ...
  {
    id: 'seo-meta',
    title: 'SEO & Meta Studio',
    desc: 'Craft OpenGraph, robots.txt, sitemap, JSON-LD.',
    icon: <Search size={18} />,
    tag: 'Web',
    component: function SeoTool({ onClose, config }: any) {
      const [{ title, desc, keywords, url }, setState, shareUrl, copyShareLink] = useToolState(
        config.id,
        {
          title: 'My Awesome Page',
          desc: 'A comprehensive guide to building awesomeness.',
          keywords: '',
        ...
        ai.run({
          ...
          if (res) {
            setState(s => ({
              ...
              desc: res.description || s.desc,
              keywords: res.keywords || s.keywords,
            }));
          }
        });
      };
      ...
      return (
        <ToolShell config={config} onClose={onClose} shareMethod={copyShareLink}>
          <SplitPane
            leftConfig={{
              title: 'PAGE DETAILS',
              ...
              <div className="p-6 space-y-4 text-sm h-full overflow-auto bg-surface/30">
                <div>
                  <label className="flex justify-between text-muted mb-1">
                    <AITooltipInfo
                      prompt="Why is the SEO Meta title important and how long should it be?"
                      value={title}
                    >
                      <span>
                        Title{' '}
                        <Sparkles size={10} className="inline ml-1 text-indigo-400 opacity-50" />
                      </span>
                    </AITooltipInfo>
                    <span className={title.length > 60 ? 'text-red-400' : ''}>
                      {title.length}/60
                    </span>
                  </label>
                  <input
                    className="w-full bg-surface border border-subtle rounded p-2 text-primary focus:border-[#34F5C5] focus:outline-none transition-colors"
                    value={title}
                ...
                <div>
                  <label className="flex justify-between text-muted mb-1">
                    <AITooltipInfo
                      prompt="Why is the SEO Description important and how long should it be?"
                      value={desc}
                    >
                      <span>
                        Description{' '}
                    ...
                    <span className={desc.length > 155 ? 'text-red-400' : ''}>
                      {desc.length}/155
                    </span>
                  </label>
                  <textarea
                    className="w-full h-24 bg-surface border border-subtle rounded p-2 text-primary resize-none focus:border-[#34F5C5] focus:outline-none transition-colors"
                    value={desc}
  ...
  {
    id: 'http-status',
    title: 'HTTP Status Lab',
    desc: 'Codes, CORS preflight, Headers analyzer.',
    icon: <Server size={18} />,
    tag: 'Network',
        ...
        <ToolShell config={config} onClose={onClose}>
          <div className="flex w-full h-full">
            <div className="w-1/3 border-r border-subtle p-8 bg-surface/30 flex flex-col justify-center items-center h-full">
              <input
                type="text"
                className="w-full max-w-sm bg-surface border-subtle mb-4 p-4 rounded text-5xl font-mono text-primary text-center block focus:border-[#34F5C5] focus:outline-none transition-colors border"
                value={code}
                onChange={e => setCode(e.target.value)}
                placeholder="e.g. 404"
              />
              <div className="text-center text-xl text-emerald-400 font-bold mb-8 h-8">
              ...
              >
                ...
                {ai.isLoading ? 'Analyzing...' : 'Deep Dive (AI)'}
              </Button>
            </div>
            <div className="w-2/3 p-8 overflow-auto markdown-body bg-base">
              {!ai.streamData && !ai.isLoading && (
                <div className="text-muted h-full flex items-center justify-center">
                  Enter a code and click Deep Dive.
                </div>
              )}
              {ai.isLoading && !ai.streamData && (
                <div className="opacity-50 animate-pulse">Consulting the RFCs...</div>
              )}
              {ai.streamData && (
                <MarkdownPreview
                  source={ai.streamData}
