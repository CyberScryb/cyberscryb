...
But honestly? When I first tried training a model on my laptop, it sounded like a jet engine was taking off in my apartment. My cat knocked the ethernet cable out halfway through and I lost 6 hours of training. That's the part the tutorials never mention — the unglamorous, messy reality behind the clean demo slides.`,
  ...
  function runAnalysis() {
    ...
    if (typeof AIDetector === 'undefined') {
      console.warn('AIDetector not loaded');
      return;
    }
  ...
  function showPopover(e, sentence, idx) {
    ...
    // Level info
    const levelLabel =
      sentence.level === 'human'
        ? 'Human-like'
        : sentence.level === 'mixed'
          ? 'Mixed'
          : 'AI Pattern';
    const color =
      sentence.level === 'human'
    ...
    const displayConf =
      sentence.level === 'ai' ? sentence.confidence : 100 - (sentence.confidence || 0);

    // Build reasons list
    const reasonsHtml = (sentence.reasons || []).map(r => `<li>${r}</li>`).join('');
    ...
    requestAnimationFrame(() => {
      ...
      const y =
        e.clientY + 15 > window.innerHeight - rect.height
  ...
  function generateTips(sentence) {
    ...
    if (reasons.some(r => r.includes('phrase') || r.includes('contains "'))) {
      tips.push(
        'Replace commonly-flagged phrases like "it is important to note" with direct statements.'
      );
    }
    if (reasons.some(r => r.includes('hedging') || r.includes('qualif'))) {
      tips.push(
        'Remove hedge words like "arguably," "potentially," or "it could be said" — state claims directly.'
      );
    }
    ...
    if (reasons.some(r => r.includes('passive'))) {
      tips.push(
        'Rewrite in active voice — "The team built the feature" instead of "The feature was built."'
      );
    }
    if (
      reasons.some(r => r.includes('vocabulary') || r.includes('vocab') || r.includes('complexity'))
    ) {
    ...
    if (sentence.level === 'ai' && tips.length === 0) {
      tips.push(
        'Try rewriting this sentence in your own voice — add a personal perspective or specific detail.'
      );
    }
    if (sentence.level === 'mixed' && tips.length === 0) {
      tips.push(
        "This sentence has some AI-like patterns but isn't strongly flagged. Consider adding a specific example or personal touch."
      );
    }
  ...
  function updateDashboardCards(text, result) {
    ...
    if (repWords.length === 0) {
      repetitionContent.innerHTML =
        '<div style="font-size:0.82rem;color:var(--aws-text-dim);">No overused words detected ✓</div>';
    } else {
      repetitionContent.innerHTML = repWords
        .slice(0, 5)
        .map(
          w =>
