...
const AIDetector = (() => {
    ...
    const FALLBACK_PHRASES = [
        ...
        'furthermore', 'moreover', 'additionally', 'in today\'s digital age',
        'in this article', 'let\'s dive in', 'without further ado',
        'it goes without saying', 'needless to say', 'at the end of the day',
        'in the realm of', 'it is crucial to', 'it should be noted that',
        'as we delve into', 'the landscape of', 'a testament to',
        'plays a crucial role', 'navigating the complexities',
        'shed light on', 'in the ever-evolving', 'on the other hand',
        'having said that', 'with that being said', 'it is essential to',
        'it is imperative', 'serves as a reminder', 'in light of',
        'paving the way', 'a myriad of', 'a plethora of',
        'harness the power', 'leverage the potential', 'unlock the potential',
        'revolutionize the way', 'foster a culture of', 'embark on a journey',
        'tapestry of', 'multifaceted', 'holistic approach',
        'paradigm shift', 'synergy', 'cutting-edge',
        'game-changer', 'groundbreaking', 'transformative',
        'in summary', 'to summarize', 'in essence', 'delve into',
        'delve deeper', 'intricacies of', 'nuances of'
    ];
    ...
    const STYLE_MODIFIERS = {
        ...
        'casual': {
            complexityVariance: 1.0, burstiness: 1.3, phraseDensity: 1.0,
            hedgingDensity: 1.0, listElaborate: 1.0, vocabRichness: 1.0,
            uniformity: 1.0, punctuationDiversity: 1.3, absenceSpecificity: 1.0
        },
        'business': {
            complexityVariance: 0.8, burstiness: 1.0, phraseDensity: 1.0,
            hedgingDensity: 0.7, listElaborate: 0.8, vocabRichness: 1.0,
            uniformity: 0.7, punctuationDiversity: 1.0, absenceSpecificity: 1.0
        },
        'academic': {
            complexityVariance: 0.6, burstiness: 0.8, phraseDensity: 0.8,
            hedgingDensity: 0.6, listElaborate: 0.7, vocabRichness: 1.0,
            uniformity: 0.6, punctuationDiversity: 0.8, absenceSpecificity: 0.8
        },
        'legal': {
            complexityVariance: 0.5, burstiness: 0.7, phraseDensity: 0.7,
            hedgingDensity: 0.5, listElaborate: 0.6, vocabRichness: 0.8,
            uniformity: 0.5, punctuationDiversity: 0.7, absenceSpecificity: 0.7
        },
        'creative': {
            complexityVariance: 1.1, burstiness: 1.3, phraseDensity: 1.0,
            hedgingDensity: 1.1, listElaborate: 1.1, vocabRichness: 0.9,
            uniformity: 1.1, punctuationDiversity: 1.3, absenceSpecificity: 0.8
        },
        'technical': {
            complexityVariance: 0.6, burstiness: 0.8, phraseDensity: 0.9,
            hedgingDensity: 0.6, listElaborate: 0.7, vocabRichness: 0.9,
            uniformity: 0.6, punctuationDiversity: 0.8, absenceSpecificity: 0.9
        }
    };
    ...
    function autoDetectStyle(text, sentences) {
        ...
        // Academic signals
        const academicSignals = (lower.match(/\b(hypothesis|methodology|findings|literature|abstract|conclusion|therefore|thus|hence|empirical|qualitative|quantitative)\b/g) || []).length;
        if (academicSignals >= 3) return 'academic';

        // Legal signals
        const legalSignals = (lower.match(/\b(pursuant|herein|thereof|whereas|notwithstanding|aforementioned|shall|stipulate|jurisdiction)\b/g) || []).length;
        if (legalSignals >= 2) return 'legal';

        // Technical signals
        const techSignals = (lower.match(/\b(function|api|database|server|algorithm|implementation|config|deploy|repository|framework|library)\b/g) || []).length;
        if (techSignals >= 3) return 'technical';

        // Creative signals
        if (text.match(/[""][^""]+[""]/g)?.length >= 3) return 'creative'; // dialogue
        const creativeSignals = (lower.match(/\b(whispered|sighed|gazed|murmured|trembled|shadows|moonlight|darkness)\b/g) || []).length;
        if (creativeSignals >= 2) return 'creative';

        // Business signals
        const bizSignals = (lower.match(/\b(roi|kpi|stakeholder|deliverable|revenue|pipeline|quarterly|strategy|metrics|growth)\b/g) || []).length;
    ...
    function heuristicVocabRichness(text, wordCount) {
        ...
        return { score, detail: `TTR: ${ttr.toFixed(3)} (${unique.size} unique / ${words.length} total)` };
    }
    ...
    function heuristicUniformity(text, sentences) {
        ...
        return { score, detail: `Para uniformity: ${paraScore.toFixed(2)}, Start patterns: ${sentScore.toFixed(2)}` };
    }
    ...
    function heuristicPunctuationDiversity(text, wordCount) {
        ...
        return { score, detail: `Diverse punct: ${totalDiverse}, Register variance: ${registerScore.toFixed(2)}` };
    }
    ...
    function heuristicAbsenceSpecificity(text, sentences, wordCount) {
        ...
        const concreteNumbers = (text.match(/\b\d{1,4}[%$€£]|\b\d{1,2}:\d{2}\b|\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d/gi) || []).length;
        ...
        const totalSpecificity = properNounDensity * 0.4 + firstPersonDensity * 0.3 + concreteDensity * 0.3;
    ...
    function analyze(text, writingStyle = 'auto') {
        ...
        const detectedStyle = writingStyle === 'auto' ? autoDetectStyle(trimmed, sentences) : writingStyle;
        ...
        const sentenceScores = sentences.map((sentence, i) => {
            ...
            const startsWithAI = /^(In |This |The |It |These |Those |However,? |Moreover,? |Furthermore,? |Additionally,? |As |While |Although )/i.test(sentence);
            ...
            const passivePattern = /\b(is|are|was|were|be|been|being)\s+(\w+ed|written|taken|given|shown|known|seen|made|found|done)\b/i;
        ...
        if (finalScore <= 40) {
            ...
            disclaimer = 'Mixed signals — this text has some AI-like patterns but may be human-written formal text. Consider using the writing style selector for better accuracy.';
    ...
    function readability(text) {
        ...
        if (wordCount === 0 || sentenceCount === 0) {
            return { fleschKincaid: 0, gradeLevel: 'N/A', avgSentenceLength: 0, avgSyllables: 0, readingTime: '0m' };
        }
    ...
    function analyzeTone(text) {
        ...
        const tones = {
            Formal: (lower.match(/\b(therefore|consequently|furthermore|moreover|nevertheless|notwithstanding|pursuant|hereby|accordingly)\b/g) || []).length,
            Conversational: (lower.match(/\b(you|your|you're|let's|we|gonna|wanna|kinda|pretty much|honestly|basically|actually|literally)\b/g) || []).length,
            Persuasive: (lower.match(/\b(should|must|need to|have to|essential|critical|important|vital|proven|guaranteed|don't miss)\b/g) || []).length,
            Informational: (lower.match(/\b(according to|research|study|data|statistics|evidence|report|analysis|survey|findings)\b/g) || []).length,
            Emotional: (lower.match(/\b(love|hate|amazing|terrible|incredible|awful|wonderful|horrible|passionate|devastating|heartbreaking|thrilling)\b/g) || []).length,
        };
    ...
    function detectPassiveVoice(sentences) {
        const passivePattern = /\b(is|are|was|were|be|been|being)\s+(\w+ed|written|taken|given|shown|known|seen|made|found|done|gone|broken|chosen|driven|eaten|fallen|forgotten|frozen|gotten|hidden|ridden|risen|spoken|stolen|sworn|thrown|worn|woven)\b/gi;
        let passiveCount = 0;
    ...
    function detectRepetition(text) {
        ...
        const stopWords = new Set(['the', 'is', 'at', 'which', 'on', 'a', 'an', 'and', 'or', 'but', 'in', 'to', 'of', 'for', 'it', 'that', 'this', 'with', 'as', 'was', 'are', 'be', 'by', 'from', 'has', 'he', 'she', 'they', 'we', 'you', 'do', 'not', 'can', 'will', 'its', 'my', 'their', 'our', 'your', 'his', 'her', 'i', 'me', 'have', 'had', 'been', 'would', 'could', 'should', 'about', 'each', 'more', 'some', 'them', 'than', 'when', 'who', 'what', 'how', 'all', 'there', 'then', 'so', 'if', 'just', 'also']);
